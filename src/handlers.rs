...
pub async fn dashboard(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let (models, loaded, rt, host_raw, gpu_mem, local_output) = tokio::join!(
        state.lms.list_models(),
    ...
    let mut models = models.unwrap_or_default();
    models.retain(|m| local_names.contains(m.id.as_str()));
